...
export class Module implements pitcher.Builds<ModuleGraph> {
  ...
  install(graph: ModuleGraph, installed: pitcher.InstalledModules, override: boolean) {
    ...
    graph.promiseEngineProvider = pitcher.singletonProvider(graph.promiseEngineProvider)((resolve) =>resolve(this.providedPromiseEngine));

    graph.digestSizeProvider = pitcher.singletonProvider(graph.digestSizeProvider)((resolve, reject) => {
      var config = graph.configProvider.get();

      pitcher.awaitAll([config[2]], (_, err) => {
        err ? reject(err) : resolve(this.providesDigestSize(config[0]));
      });
    });
    ...
    graph.clientProvider = pitcher.singletonProvider(graph.clientProvider)((resolve, reject) => {
      ...
      var promiseEngine = graph.promiseEngineProvider.get();

      pitcher.awaitAll([config[2],isSandbox[2],parsedCommandLine[2],promiseEngine[2]], (_, err) => {
        err ? reject(err) : resolve(this.providesClient(config[0], isSandbox[0], parsedCommandLine[0], promiseEngine[0]));
      });
    });
    ...
    graph.unfoundNotebookNamesProvider = pitcher.singletonProvider(graph.unfoundNotebookNamesProvider)((resolve, reject) => {
      var sourceNotebooks = graph.sourceNotebooksProvider.get();
      var config = graph.configProvider.get();

      pitcher.awaitAll([sourceNotebooks[2],config[2]], (_, err) => {
        err ? reject(err) : resolve(this.providesUnfoundNotebookNames(sourceNotebooks[0], config[0]));
      });
    });
...
export class InferredModuleGraph {
  ...
  promiseEngineProvider = pitcher.typeOfGiven(Module.prototype.providedPromiseEngine);
  digestSizeProvider = pitcher.typeOfProvider(Module.prototype.providesDigestSize);
  ...
  unfoundNotebookNamesProvider = pitcher.typeOfProvider(Module.prototype.providesUnfoundNotebookNames);
